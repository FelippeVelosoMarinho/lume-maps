...
export function PassportTravelsMap({ journeys, className }: Props) {
  ...
  const hasPlanning = useMemo(() => journeys.some((j) => j.is_planning), [journeys])

  const visibleJourneys = useMemo(
    () => (showPlanning ? journeys : journeys.filter((j) => !j.is_planning)),
    [journeys, showPlanning],
  )
  ...
  if (!withPath.length && !journeys.some((j) => j.markers.length > 0)) {
    return (
        ...
        {withPath.map((j) => (
          <li key={j.id} className="flex items-center gap-2">
            <span
              ...
              style={{
                background: j.color,
                opacity: j.is_planning ? PLANNING_MAP_OPACITY : 1,
              }}
              aria-hidden
            />
            ...
            {j.is_planning && (
              <span className="text-[10px] uppercase tracking-wide text-earth/80">planejamento</span>
            )}
