...
export function JourneyEditMenu({
  ...
  return (
    <div className="map-ui-overlay !pointer-events-none">
      <button
        type="button"
        className="pointer-events-auto absolute inset-0 bg-ink/25 md:bg-ink/10"
        aria-label="Fechar menu"
        onClick={onClose}
      />
      <div className="pointer-events-auto absolute inset-x-0 bottom-0 md:inset-y-3 md:right-3 md:left-auto md:bottom-3 md:w-[min(400px,calc(100%-1.5rem))] max-h-[min(85dvh,36rem)] md:max-h-[calc(100%-1.5rem)] overflow-y-auto rounded-t-2xl md:rounded-sm border border-ink/25 bg-paper shadow-2xl paper-grain doc-frame pb-[env(safe-area-inset-bottom)]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <label className="block text-sm min-w-0">
                ...
                <input
                  ...
                  className="mt-1 w-full min-w-0 border border-dashed border-ink/30 bg-cream px-2 py-2 text-sm"
                />
              </label>
              <label className="block text-sm min-w-0">
