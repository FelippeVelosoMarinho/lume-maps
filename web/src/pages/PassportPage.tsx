...
import {
  hasPublicProfileContent,
  PassportPrivacyModal,
  type PrivacyFlags,
} from '../components/PassportPrivacyModal'
...
export function PassportPage() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((p) => {
        ...
        setPassport(p)
        const wantTravels =
          isOwner || p.public_show_travels_map !== false
        if (wantTravels) {
          api
            .getPassportTravels(username)
            .then((t) => {
              if (!cancelled) setTravels(t)
            })
            .catch(() => {
              if (!cancelled) setTravels(null)
            })
        } else {
          setTravels(null)
        }
  ...
  async function savePrivacy(flags: PrivacyFlags) {
    setBusy(true)
    setError('')
    try {
      const updated = await api.updatePassport(flags as Partial<Passport>)
      setPassport((p) =>
        p
          ? {
              ...p,
              ...updated,
              stamps: updated.stamps?.length ? updated.stamps : p.stamps,
              journeys: updated.journeys?.length ? updated.journeys : p.journeys,
            }
          : p,
      )
      if (username) {
        const p = await api.getPassport(username)
        setPassport(p)
        if (flags.public_show_travels_map || isOwner) {
          const t = await api.getPassportTravels(username).catch(() => null)
          setTravels(t)
        }
      }
      await refresh()
  ...
  // Visitante sem nada liberado → landing de convite
  if (passport && !isOwner && !hasPublicProfileContent(passport)) {
    return (
      <Shell compact>
  ...
  const showTravelsMap = isOwner || passport?.public_show_travels_map !== false
  const showStamps = isOwner || passport?.public_show_stamps !== false
  const showJourneys = isOwner || passport?.public_show_journeys !== false
          ...
          <>
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <h2 className="font-display text-2xl uppercase tracking-wide text-ink">
                {isOwner ? 'Meu passaporte' : `Passaporte de ${passport.display_name}`}
              </h2>
              <div className="flex flex-wrap items-center gap-2">
                {isOwner ? (
                  ...
                  <>
                    {!me && (
                      <Link
                        to={`/auth?mode=signup&next=${encodeURIComponent(`/p/${passport.username}`)}`}
                        className="inline-flex items-center justify-center rounded-xl bg-earth text-cream px-5 py-2.5 text-sm font-medium shadow-sm hover:brightness-110 min-h-11"
                      >
            ...
            {isOwner && editing ? (
              ...
              <PassportCard
                passport={passport}
                onPhotoChange={isOwner ? onPhoto : undefined}
              />
            )}
                    ...
                    <div key={j.id} className="doc-frame bg-cream p-4 relative group min-w-0">
                      <Link to={`/v/${j.slug}`} className="block hover:opacity-90 min-w-0">
                        ...
                        <p className="text-sm text-earth mt-1 line-clamp-2">
                          {j.subtitle
                            ? stripMarkdown(j.subtitle) || 'Sem descrição'
                            : 'Sem descrição'}
                        </p>
                        ...
                        <span className="text-xs text-earth mt-2 inline-block">
                          {isOwner && j.is_mine !== false ? 'Abrir / editar →' : 'Abrir mapa →'}
                        </span>
                      </Link>
                      {isOwner && (
